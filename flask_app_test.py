...
def require_admin(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not user or user.get('role') != 'admin':
            return jsonify({"error": "غير مصرحØŒ Ù„Ù„Ù…Ø¯ÙŠØ± ÙÙ‚Ø·"}), 403
...
def user_profile():
    user = get_session_user()
    if not user:
        return jsonify({"error": "غير مصرحØŒ Ø§Ù„Ø±Ø¬Ø§Ø¡ ØªØ³Ø¬ÙŠÙ„ Ø§Ù„Ø¯Ø®ÙˆÙ„"}), 401
    conn = sqlite3.connect(DB_FILE, timeout=30)
    try:
        c = conn.cursor()
...
def get_admin_config():
    user = get_session_user()
    if not user or user['role'] != 'admin':
        return jsonify({"error": "غير مصرحØŒ Ù„Ù„Ù…Ø³Ø¤ÙˆÙ„ÙŠÙ† ÙÙ‚Ø·"}), 403
    try:
        conn = sqlite3.connect(DB_FILE, timeout=30)
        try:
            c = conn.cursor()
...
def save_manga():
    user = get_session_user()
    if not user or user['role'] != 'admin':
        return jsonify({"error": "غير مصرحØŒ ÙŠØ¬Ø¨ Ø£Ù† ØªÙƒÙˆÙ† Ø§Ù„Ù…Ø¯ÙŠØ± Ù„Ù„Ù‚ÙŠØ§Ù… Ø¨Ù‡Ø°Ø§ Ø§Ù„Ø¹Ù…Ù„"}), 403
